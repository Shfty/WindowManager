...
#include <QDebug>
Q_LOGGING_CATEGORY(treeIconImageProvider, "shared.treeIconImageProvider")

TreeIconImageProvider::TreeIconImageProvider()
	:QQuickImageProvider(QQuickImageProvider::Pixmap)
	, m_emptyPixmap(QPixmap(1,1))
{
	qCInfo(treeIconImageProvider, "TreeIconImageProvider Startup");

	qCInfo(treeIconImageProvider, "Creating empty pixmap");

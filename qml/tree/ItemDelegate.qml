...
Item {
    ...
    Item {
        ...
        Rectangle {
            ...
            property var settingsContainer: appCore.settingsContainer
            color: settingsContainer.colorContainerPlaceholder
        }

        ManagedWindow {
            anchors.fill: parent
            hwnd: model ? model.hwnd : null
            parentHwnd: appCore.windowView.getWindowHwnd(Window.window)
            clipTarget: {
                if(!model) return null
                if(model.treeParent == null) return null
                if(model.treeParent.layout != "Tabbed") return null;
